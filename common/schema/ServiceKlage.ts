...
export const privatpersonSchema = zod.object({
	...
	innmelder: zod.object({
		...
		personnummer: zod.string().regex(/^\d{11}$/, 'Personal ID number must be 11 digits'),
	}),
});
...
export const bedriftSchema = zod.object({
	...
	paaVegneAvBedrift: zod.object({
		navn: zodString,
		organisasjonsnummer: zod.string().regex(/^\d{9}$/, 'Organization number must be 9 digits'),
...
export const serviceKlageSchema = serviceKlageBaseSchema.and(serviceKlageFragmentSchema).refine(
	(data) => {
	...
	{
		message: 'oenskerAaKontaktes must be true when paaVegneAv is BEDRIFT',
